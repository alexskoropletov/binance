...
export class Beautifier {
  ...
  beautify(object: any, type: string) {
    const beautifications = _.isObject(type)
      ? type
      : this.beautifications[type];
    if (!beautifications) {
      return object;
    }
    const newItem: any = {};
    _.each(object, (value: any, key: string) => {
      ...
      if (_.isArray(value)) {
        const newArray: any[] = [];
        if (_.isArray(this.beautifications[beautifications[key]])) {
          _.each(value, (arrayValue: any) => {
            newArray.push(
              this.beautify(
                arrayValue,
          ...
          _.each(value, (arrayValue: any, arrayKey: string) => {
            newArray.push(
